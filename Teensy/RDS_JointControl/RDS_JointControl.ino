...
// --- Configuration ---
#define SPI_CS_PIN          10            // User specified Pin 10
#define SPI_SCLK_FREQUENCY  10000000      // 10MHz
#define SPI_MODE            SPI_MODE0     // Starting with Mode 0 for stability
#define UART_BAUDRATE       115200

#define READ_REG_COMMAND    (0b010 << 13)
#define WRITE_REG_COMMAND   (0b100 << 13)

// Pre-define settings for efficiency
SPISettings maSettings(SPI_SCLK_FREQUENCY, MSBFIRST, SPI_MODE);
...
void setup() {
  ...
  while (!Serial); // Wait for Teensy Serial to wake up
  
  SPI.begin();

  Serial.println("--- MA782 Initialization ---");

  // Read initial zero settings
  uint8_t reg0 = readRegister(0);
  uint8_t reg1 = readRegister(1);
  Serial.printf("Initial Reg[0]: %d | Reg[1]: %d\n", reg0, reg1);

  // Attempt to write new zero settings (0x7FF)
  // Note: 0x7FF is split across Reg 0 (lower 8 bits) and Reg 1 (upper 3 bits)
  uint8_t wReg0 = writeRegister(0, 0xFF);
  uint8_t wReg1 = writeRegister(1, 0x7F);

  if (wReg0 == 0xFF && wReg1 == 0x7F) {
    Serial.println("SUCCESS: Write Verified.");
  } else {
    Serial.println("ERROR: Write Failed or MISO disconnected.");
  }
}

void loop() {
  double currentAngle = readAngleInDegree();
...
uint16_t readAngle() {
  uint16_t angle;
  SPI.beginTransaction(maSettings);
...
uint8_t readRegister(uint8_t address) {
  ...
  SPI.beginTransaction(maSettings);
  // Phase 1: Send Read Command
  digitalWrite(SPI_CS_PIN, LOW);
  SPI.transfer16(READ_REG_COMMAND | ((address & 0x1F) << 8));
  digitalWrite(SPI_CS_PIN, HIGH);
  
  delayMicroseconds(1); // Brief dwell for sensor processing

  // Phase 2: Read back the result
  digitalWrite(SPI_CS_PIN, LOW);
  response = SPI.transfer16(0x0000);
  digitalWrite(SPI_CS_PIN, HIGH);
  SPI.endTransaction();
  
  return (uint8_t)((response & 0xFF00) >> 8);
}
...
uint8_t writeRegister(uint8_t address, uint8_t value) {
  ...
  delay(25); // MA782 requires ~20ms to burn to non-volatile memory

  // Phase 2: Read back to verify
  digitalWrite(SPI_CS_PIN, LOW);
  response = SPI.transfer16(0x0000);
  digitalWrite(SPI_CS_PIN, HIGH);
  SPI.endTransaction();
