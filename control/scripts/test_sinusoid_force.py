...
print("\nWARNING: Ensure the finger is pressing against a solid surface/load cell before starting!")
print("Starting in 3 seconds...")
time.sleep(3)

# Send command to trigger the chirp
print("Triggering 60-second Force Sine Chirp Sweep...")
ser.write(b"TEST SINE FORCE LOW\n") # Ensure this triggers testSineForce() in Command.ino

times, expected_f, actual_f = [], [], []
recording = False
    ...
    if recording and line:
        try:
            ...
            if len(parts) == 3:
                ...
                expected_f.append(float(parts[1]))
                actual_f.append(float(parts[2]))
        except ValueError:
            pass 
...
exp_f = np.array(expected_f)
act_f = np.array(actual_f)
...
for f_target in target_freqs:
    ...
    t_slice = t_arr[mask]
    exp_slice = exp_f[mask]
    act_slice = act_f[mask]
    
    if len(t_slice) < 20: continue
    
    try:
        # Dynamically guess amplitude and offset to handle both 1N and 10N tests
        amp_guess = (np.max(exp_slice) - np.min(exp_slice)) / 2.0
        offset_guess = np.mean(exp_slice)
        
        # Fit Expected (Reference) 
        popt_exp, _ = curve_fit(chirp_model, t_slice, exp_slice, p0=[amp_guess, 0.0, offset_guess])
...
ax1.set_title(f'Bode Plot: Force Tracking Frequency Response')
...
if bandwidth:
    print(f"\n✅ Force Control Bandwidth (-3dB dropoff) is approximately {bandwidth:.2f} Hz.")
else:
